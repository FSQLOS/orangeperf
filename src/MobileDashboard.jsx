...
export default function MobileDashboard({ config }) {
    ...
    const TROPHY_TITLES = {
        ...
        MEV: { title: "Le MacGyver des Options", sub: "Il te rajoute du contenu en un clin d'œil." },
    ...
    const openGlobalComparison = (category) => {
        ...
        if (data[0] && data[0].val > 0) {
            playTrophySound();
            setActiveTrophy({ name: data[0].name, category, title: TROPHY_TITLES[category]?.title, sub: TROPHY_TITLES[category]?.sub });
    ...
    if (loading) return <div className="loading-screen"><div className="ps5-loader"></div><div className="loading-text">Chargement du profil...</div></div>;
    ...
    return (
        ...
        {/* CLASSEMENT */}
        <div className="section-label" style={{marginTop:'20px'}}>🏆 Classement CA ACC</div>
        ...
        {caModal && (
            ...
            <div className="modal-header"><h2>Pilotage CA HT</h2><X onClick={() => setCaModal(false)}/></div>
            <div className="ro-main-stat">
            <div className="ro-label">Réalisé au {mInfo.now}</div>
        ...
        {/* LE TROPHÉE : RENDU EN DERNIER POUR ÊTRE AU PREMIER PLAN */}
        {activeTrophy && (
