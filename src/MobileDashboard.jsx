...
export default function MobileDashboard({ config }) {
    ...
    // Initialisation sécurisée pour éviter le crash "counts undefined"
    ...
    const KEY_NOT_TERM = ["COQUE", "ETUI", "VERRE", "FILM", "PROT", "CHARGEUR", "CABLE", "BUDS", "AIRPODS"];
    const BLACKLIST_CA = ["FIXE", "DECT", "GIGASET", "PILE", "SAC", "KRAFT", "FLASH", "EXPERTE", "ATELIER"];
    ...
    const processData = (data) => {
        ...
        data.forEach(row => {
            ...
            if (v) {
                ...
                // --- LOGIQUE CALCUL CA ACCESSOIRES ---
                const isTerm = (KEY_STOCKAGE.some(k => lib.includes(k)) || KEY_MODELE.some(k => lib.includes(k))) && !KEY_NOT_TERM.some(k => lib.includes(k));
                ...
                if (isToday) {
                    tDay[v].CA += ht;
                    if (!tDay[v].tickets[ticketId]) tDay[v].tickets[ticketId] = { date, items: [] };
                    tDay[v].tickets[ticketId].items.push({ lib, fam: getFamily(lib, codeArt), ca });
    ...
    // --- RENDU ---
    if (loading) return <div className="loading-screen"><div className="loader"></div><p>Synchronisation en cours...</p></div>;
    ...
    return (
        ...
        {caModal && (
            ...
            <div className="ro-footer-card" style={{background:'#f8fafc', padding:'15px', borderRadius:'15px', marginTop:'15px'}}>
            <div style={{display:'flex', justifyContent:'space-between'}}><span>Objectif Mois</span><strong>{config.objectifs['CA'] || 0} €</strong></div>
            <div style={{display:'flex', justifyContent:'space-between', marginTop:'5px'}}><span>Atterrissage</span><strong style={{color: landingCA >= (config.objectifs['CA'] || 0) ? '#10b981' : '#f59e0b'}}>{landingCA} €</strong></div>
            </div>
            </div>
            </div>
            </div>
        ...
        {compareMode && (
            ...
            options={{ indexAxis: 'y', responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, datalabels: { anchor: 'end', align: 'right', offset: 4, color: '#1a1a1a', font: { weight: 'bold' }, formatter: v => v + (compareMode.isPercent ? '%' : '') } }, scales: { x: { display: false }, y: { grid: { display: false }, ticks: { font: { weight: 'bold' } } } } }} />
            </div>
            </div>
            </div>
        )}

        {/* MODALE DÉTAILS VENDEUR (FACTURATION PRO) */}
        {selectedSeller && (
            ...
            <div className="modal-header"><h2>Détails : {selectedSeller.name}</h2><X onClick={() => setSelectedSeller(null)}/></div>
